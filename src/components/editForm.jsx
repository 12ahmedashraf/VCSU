...
export default function EditForm({item , type , edit})
{
    const initialState = {status: 'pending',message : ''};
      ...
      const [editStatus,formAction,isPending] = useActionState(SubmitProblem,initialState);
        return(<>
        { type === 'problem' ? (<>
        {(edit === true) ? (
        <div className="rounded-2xl flex flex-col flex-1 ">
            ...
            <div className="submitt flex justify-center">
                { editItem ? 
            <button type="submit" onClick={()=> {setEditItem(null)}} className="font-anton bg-black text-white border-black rounded-2xl border-2 px-4 py-2 hover:scale-110 transition-all duration-300 hover:cursor-pointer ">
                Submit
            </button> : <button   type="button" onClick={()=> {setEditItem(item)}} className="font-anton bg-black text-white border-black rounded-2xl border-2 px-4 py-2 hover:scale-110 transition-all duration-300 hover:cursor-pointer ">
                Edit
            </button>
            }
            </div>
            {(!isPending) && (<p className={`font-league text-center  ${editStatus.status === 'success' ? 'text-green-500' :'text:red-500'}`}><strong>{editStatus.message}</strong></p>)
            }
            </form>
        </div>):(<></>)}
</>):(<></>)}
