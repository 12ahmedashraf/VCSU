...
export default  function UserLogs({problem})
{
   const [logs, setLogs] = useState([]);
    ...
    return((logs.length==0) ? (<p className="flex flex-col font-bold my-5 font-league px-10 gap-2.5 rounded-2xl">We are still reviewing your problem!</p>)
        :(<div className="flex flex-col my-5 font-league gap-2.5 mx-5 rounded-2xl font-bold">
            {
                logs.map((item) => (
                    (!item.is_internal) ? (
                    <div key={item.id} className="flex flex-1 justify-between items-center">
                        <h4 className="w-[70%] break-all">{item.note}</h4>
                        <h4 className="w-[15%] break-all flex justify-center text-center">{formatDate(item.created_at)}</h4>
                    </div>     ):(<></>)
                ))
            }
        </div>)
